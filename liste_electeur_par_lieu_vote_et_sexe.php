...
<body>
    ...
    <center style="margin-top: 50px;" id="id_list_deroulante_vs"> 
    <label for="">Lieu de vote: </label>
    <select name="lieu" id="lieu" required>;
        <?php
                
                $statement =  $db->prepare("SELECT DISTINCT LIEU_VOTE FROM electeur");
                $statement->execute();
                ...
                echo '<option value="">selectionner un lieu</option>';
                    ...
                    foreach ($clients as $key => $value) {
                        ...
                        echo '<option value="'.$value['LIEU_VOTE'].'">'. $value['LIEU_VOTE'] .'</option>';
                    }
           ...
           <select name="sexe" id="sexe"> 
                ...
                <option value="F">F</option>
                     ...
                     <center>
                         <br/><br/>
                       <h3 style="margin-top: 30px;">LISTE DES ELECTEURS PAR QUARTIER ET SEXE </h3></center>
...
document.querySelector('#lieu').addEventListener('change', function(){
  document.querySelector('#table').innerHTML = '';
  i = 1;
  let data = {
    lieu : document.querySelector('#lieu').value
  }
  xhr.open('POST', 'liste_lieu_vote.php', true);
...
document.querySelector('#sexe').addEventListener('change', function(){
  ...
  xhr.open('POST', 'lieu_sexe.php', true);
  xhr.setRequestHeader('Content-type', 'application/json; charset=UTF-8');
  xhr.send(JSON.stringify(data));
